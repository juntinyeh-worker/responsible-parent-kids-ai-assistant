...
echo "  aws ecs describe-services --cluster child-voice-tutor --services child-voice-tutor-svc --region $REGION --query 'services[0].events[:5]'"
echo ""
echo "  # Tail container logs"
echo "  aws logs tail /ecs/child-voice-tutor --follow --region $REGION"
echo ""
echo "  # Force new deployment (after pushing a new image)"
echo "  aws ecs update-service --cluster child-voice-tutor --service child-voice-tutor-svc --force-new-deployment --region $REGION"
echo ""
echo "  # Tear down"
